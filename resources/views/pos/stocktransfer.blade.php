...
<section class="section">
      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">Inventory Transfer Report</h5>
              
              

              @if(auth()->user()->can('warehouse.transferstocklog') )
              <button class="btn btn-info" onclick="ExportToExcel('xlsx')">Export table to excel</button>
              @endif
              ...
              <table class="table datatable "  id="tbl_exporttable_to_xls">
                ...
                @endif
                    ...
                    <td>{{ $rec->proname }}</td>
                    <td>{{ $rec->quantity }}</td>
                    <td>{{ getbranchnamebyid($rec->frombranchId) }}</td>
                    <td>{{ getbranchnamebyid($rec->tobranchId) }}</td>
                    <td>{{ $rec->username }}</td>
...
<div class="modal-dialog">
 <div class="modal-content">
   <div class="modal-header">
     <h5 class="modal-title">Inventory Transfer Report</h5>
     <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
   </div>
   <div class="modal-body">
   
   <form name="" action="{{ route('pos.transferstocklog') }}" method="GET" class="row g-3">
...
<p>Branch</p>
<select name="branchId" required class="form-select"  aria-label="State">
